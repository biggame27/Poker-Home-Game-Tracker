...
import { getGamesByUser, getGroups, getPlayerProfile } from '@/lib/supabase/storage'
import type { Game, Group, GameSession, PlayerProfile } from '@/types'
...
export default function PlayerProfilePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setLoading(true)
      try {
        const [player, playerGames, playerGroups] = await Promise.all([
          getPlayerProfile(clerkId),
          getGamesByUser(clerkId),
          getGroups(clerkId),
        ])
        setProfile(player)
  ...
  const sessions = useMemo<GameSession[]>(() => {
    return games.flatMap((game) => game.sessions.filter((s) => s.userId === clerkId))
  }, [games, clerkId])
  ...
  const badges = useMemo(() => {
    ...
    const daySet = Array.from(
      new Set(
        games
          .filter((g) => g.sessions.some((s) => s.userId === clerkId))
    ...
    const badgesList: { title: string; description: string; icon: React.ReactNode }[] = []
    ...
    return badgesList
  }, [games, sessions, stats.totalSessions, stats.wins, stats.winRate, clerkId])

  const historyRows = useMemo(() => {
    return games
      .map((game) => {
        const session = game.sessions.find((s) => s.userId === clerkId)
  ...
  const filteredHistory = useMemo(() => {
    ...
    const filtered = query
      ? historyRows.filter((row) => {
          ...
          return row.groupName.toLowerCase().includes(query) || dateText.includes(query)
        })
      : historyRows
  ...
  const groupBreakdown = useMemo(() => {
    return groups
      .map((group) => {
        ...
        const sessionsInGroup = gamesInGroup.flatMap((g) =>
          g.sessions.filter((s) => s.userId === clerkId)
        )

        const profit = sessionsInGroup.reduce(
        ...
        const myRankEntry = sorted.find((entry) => entry.id === clerkId)
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card>
          <CardContent className="p-6">
            ...
            <div className="mt-4">
              <Link href="/players">
                <CardDescription className="underline">Back to search</CardDescription>
              </Link>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-4">
            <Avatar className="h-16 w-16">
              <AvatarFallback>
                {profile.fullName?.[0] || profile.clerkId?.[0] || '?'}
              </AvatarFallback>
            </Avatar>
            <div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <CardDescription>Profit trends over time and per game</CardDescription>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <RunningTotalsChart
              ...
              description="Bankroll over time"
            />
            <RunningTotalsChart
              games={games}
              cumulative={false}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Groups this player belongs to</CardDescription>
          </CardHeader>
          <CardContent>
            {groups.length === 0 ? (
                ...
                {groups.map((group) => (
                  ...
                  >
                    ...
                    <Link href={`/groups/${group.id}`}>
                      <CardDescription className="underline">View Group</CardDescription>
                    </Link>
                  </div>
                ))}
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Profit and rank within each group</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <CardDescription>Every game with buy-in and profit</CardDescription>
            </div>
        ...
        <Card>
          <CardHeader className="flex flex-col gap-2">
            ...
            <CardDescription>Best matchups and closest rivals based on shared games</CardDescription>
          </CardHeader>
          <CardContent>
            {matchups.length === 0 ? (
